...
$(document).ready(function(){/* 날짜 설정 */
	$("select option[value='${book.getBookClass()}']").attr('selected','true');
});
</script>
</head>
<body id="page-top">
...
<c:if test="${updateCnt==0 }">
  ...
  <!-- Page Wrapper -->
 	<div id="wrapper">
 		<c:set var="where" value="${path }/host/01editStock"/>
        ...
        <div class="container-fluid">
	          ...
	          <!-- Page Heading -->
	          <h1 class="h3 mb-2 text-gray-800">책 수정하기</h1>
	          <c:if test="${ISBN == null}">
		          <script type="text/javascript">
		          	var bookISBN ="";	
		          	while(bookISBN.length < 13){
	          			if(bookISBN != "") bookISBN= prompt("수정할 책의 ISBN을 입력하세요","잘못입력하셨습니다 다시 입력하세요");
	          			else bookISBN= prompt("수정할 책의 ISBN을 입력하세요","13자리입니다");
		          	}
		          	location.href="${path}/host/01editStock?ISBN="+bookISBN;
		          </script>
			  </c:if>
			<c:if test="${ISBN != null}">
			
	          <!-- DataTales Example -->
	          <form action="${path }/host/01editStockPro?${_csrf.parameterName}=${_csrf.token}" name="stockList" method="post" enctype="multipart/form-data">
	        	<input type = "hidden" name="${_csrf.parameterName}" value = "${_csrf.token}">
	        	<input type="hidden" name="originImg" value="${book.getBookImage() }">
	          <div class="row">
	          ...
	          <div class="col-xl-4 col-md-6 mb-4"><!-- 크기조절  -->
	          	<div class="card shadow mb-4">
	                ...
	                <div class="card-body">
	                	<div id="holder"><img id="previewImg" width="300" height="450px" src="${image }${book.getBookImage() }"></div>
	          ...
	          <div class="card shadow mb-4">
	            <div class="card-body">
					...
					<tr>
						...
						<td><input type="text" size="30" name="bookName" value="${book.getBookName() }"></td>
					</tr>
					<tr>
						<th><label for="bookWriter"> 작가 : </label></th>
						<td><input type="text" size="30" name="bookWriter" value="${book.getBookWriter() }"></td>
					</tr>
					<tr>
						<th><label for="bookPublisher"> 출판사 : </label></th>
						<td><input type="text" size="30" name="bookPublisher" value="${book.getBookPublisher() }"></td>
					</tr>
					<tr>
						<th><label for="publishDate"> 출판일 </label></th>
						<td><input type="date" size="30" name="publishDate" id="publishDate" value='${book.getPublishDateForString() }'/></td>
					</tr>
					<tr>
						<th><label for="bookPrice"> 정가 </label></th>
						<td><input type="number" size="30" name="bookPrice" size="30" value="${book.getBookPrice() }"></td>
					</tr>
					<tr>
						<th><label for="sale"> 판매가 </label></th>
						<td><input type="number" size="30" name="sale" size="30" value="${book.getSale() }"></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input type="text" name="page"size="30" value="${book.getPage() }"></td>
					</tr>
					<tr>
						<th><label for="ISBN"> ISBN </label></th>
						<td><input type="text" name="ISBN"size="30" value="${book.getISBN() }" readonly></td>
					</tr>
					<tr>
						<th><label for="bookCount">수량</label></th>
						<td><input type="number" name="bookCount" value="1" min="0" max="1000" step="1" size="20"  value="${book.getBookCount() }"></td>
					</tr>
					<tr>
						<td colspan="2" id="bookButton">
							<input type="submit" value="수정하기">
							<input type="button" value="삭제하기" onclick="window.location='${path }/host/01deleteStockPro?ISBN=${ISBN}'">
						</td>
					</tr>
	                  </tbody>
	                </table>
